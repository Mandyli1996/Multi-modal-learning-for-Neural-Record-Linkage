...
def gru( units,dropout, directional=True, return_sequences= True, gpu= 1 ):
...
#         similarity_image_layers.append(similarity_image_layer)
    ...
    gru_1 = gru(units=300,dropout=0.4, directional=True, return_sequences = True)
#     units=300
#     dropout=0.4
#     directional=True
#     return_sequences = True
#     rnn_1_layer = keras.layers.GRU(units, dropout= dropout,
#                                    return_sequences=return_sequences, 
#                                    recurrent_activation='sigmoid', 
#                                    recurrent_initializer='glorot_uniform')
    ...
    g1['left'] = rnn_1_layer(text_img_tensors['left'])
    g1['right'] = rnn_1_layer(text_img_tensors['right'])
    ...
    gru_2 = gru(units=300,dropout=0.4, directional=True, return_sequences= False)
#     units=300
#     dropout=0.4
#     directional=True
...
#                                    recurrent_initializer='glorot_uniform')
    ...
    g2 = dict()
    g2['left'] = rnn_2_layer(text_img_tensors['left'])
    g2['right'] = rnn_2_layer(text_img_tensors['right'])
...
histories = dict(acc=list(), val_acc=list(), loss=list(), val_loss=list(), precision=list(), val_precision=list(), recall = list(), val_recall=list(), f1_score=list(), val_f1_score=list(), auroc =list(), val_auroc=list())
...
earlystop = keras.callbacks.EarlyStopping(monitor='val_auroc', min_delta=0, patience=4, verbose=1, mode='max', baseline=None, restore_best_weights=True)
...
history = model.fit_generator(generator=training_generator, steps_per_epoch=len(X_train[1])//100 , epochs=10, validation_data=validation_generator, validation_steps = len(X_val[1])//100,
                    workers=10, use_multiprocessing = True, max_queue_size=20, verbose =1, shuffle=True, callbacks = [earlystop])
...
with open('./history_mymodel_imgadvanced.map', 'wb') as f:
